...
setwd('D:/DLIANG/School/Undergraduate/Semester 3/STAT 184 (R Resources)')
...
#ggplots
ggplot(aggDT, aes(x=mean_commentscore, y=post_score))+geom_point()+labs(title='Post Score vs. Mean Comment Score',caption='This plot is intended to visualize how the score of each post relates to the mean score of their comments')+geom_abline(aes(intercept=-1309.61,slope=83.19))
ggplot(DT, aes(x=com_postAge, y=comment_score))+geom_point()+labs(title='Comment Score vs. Post Age in Days at Comment Submission', caption='This plot is intended to visualize how popular a comment gets based on how long it is posted after the post it is under')+geom_abline(aes(intercept=120.150,slope=-9.477))
